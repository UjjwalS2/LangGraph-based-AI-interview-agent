...
# The 8 nodes exactly as named in the README's architecture diagram, so
# this tracker lines up 1:1 with real LangGraph node names when you
# switch from the demo engine to your compiled graph.
PIPELINE_NODES = [
...
def render_pipeline(active_key: str | None, done_keys: set[str], skipped_keys: set[str],
                     status_keys: dict[str, str] | None = None) -> None:
    """Render the 8-node pipeline strip.

    status_keys optionally maps node key -> 'miss' | 'escalate' to tint a
    completed chip (e.g. cache miss, escalation) instead of the default
    neutral 'done' color.
    """
...
def confidence_gauge(value: float) -> go.Figure:
    """Small gauge, 0-1 confidence score, threshold line at 0.72 (the
    quality-gate cutoff from the README)."""
...
def competency_radar(scores: dict[str, float]) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            fill="toself",
            fillcolor=f"rgba(69, 214, 196, 0.18)",
    ...
    fig.update_layout(
        polar=dict(
            ...
            radialaxis=dict(
                visible=True, range=[0, 10], gridcolor=COLORS["border"],
                tickfont={"size": 9, "color": COLORS["text_faint"]},
            ),
            angularaxis=dict(
                gridcolor=COLORS["border"],
                tickfont={"family": FONT_UI, "size": 12, "color": COLORS["text"]},
            ),
        ),
        showlegend=False,
        paper_bgcolor="rgba(0,0,0,0)",
